...
# 使用PLBART生成各文件源码中被重构函数的各代码行生成的文本向量
# 其中一些文件的源码为空 或者是 有效代码行为空   这些文件不会生成对应向量文件

np.set_printoptions(suppress=True)
ss = preprocessing.StandardScaler()
print('Start To Load Pretrain Model ... ...')
tokenizer = AutoTokenizer.from_pretrained(r"../../REMS/model/plbart-base")
model = AutoModel.from_pretrained(r"../../REMS/model/plbart-base")
    ...
    if len(codes) != 0:
        ...
        for line in codes:
            ...
            if line in ['\n', '\r\n'] or line.strip() == "":
                ...
                vecs.append(vec)
        # 将各行代码嵌入向量写入文件
        output_fn = set_path + '/code_embedding/PLBART_vec.csv'
